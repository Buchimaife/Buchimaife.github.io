...
test.subset <- sample(900, 300)     # we randomly choose 300 indices for test data 
...
#Machine Learning Part (c)∗∗∗: Select two supervised classification methods that you
#studied on this module that would be suitable for these data. Briefly justify your choice.
...
lda.test.pred <- predict(pur.lda, newdata = customers.test)  # predictions for test data
lda.test.class <- lda.test.pred$class  # predicted classes for the test data
# when threshold equal to 0.5 is used.
lda.test.tab <- table(lda.test.class, customers.test$purchase)   # confusion matrix
lda.test.tab   
...
new.test.tab <- table(new.classes, customers.test$purchase)
new.test.tab   
...
# because of the way the data is distributed. However, we are able to observe
...
#From the comparison, QDA is more suitable for this classification problem 
#Assuming a normal distribution then the straight line is where the two densities
#intercept and this will be the class boundary but in this case the variability is not the same therefore 
...
#Values closer to 1 means a good predictive capability. QDA is more flexible. The bias-variance trade-off for  LDA although being 
#more stable (lower variance) , is less flexible and may be biased. 
